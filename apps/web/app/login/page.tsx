...
export default function LoginPage() {
  ...
  return (
    <main className="max-w-md mx-auto px-6 py-16">
      <SpecCard label="ACCESS CONTROL / SIGN-IN" variant="bone">
        {supabaseConfigured ? (
          <LoginForm />
        ) : (
          <form action={devLoginAction} className="space-y-4">
            ...
            <button
              type="submit"
              className="w-full bg-ink text-hazard spec-label font-bold py-3 border border-ink hover:bg-hazard hover:text-ink transition-colors"
            >
              CREATE DEV OPERATOR →
            </button>
          </form>
